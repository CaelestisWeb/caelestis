...
```bash
node identite/recherche/pistes-pousse/build-pistes.mjs     # les 55 fichiers, puis le contrôle de cadrage
node identite/recherche/pistes-pousse/build-planche.mjs    # la planche de présentation
```

Le code de fabrication est commun aux trois séries : `identite/recherche/commun/fabrique.mjs` écrit les fichiers, mesure leur cadrage et monte la planche ; `identite/recherche/commun/artefacts.mjs` compose signe, tuile et lockups ; `identite/recherche/commun/planche.css` habille les trois planches. Une série ne fournit que ses signes, ses repères de construction et son texte.
