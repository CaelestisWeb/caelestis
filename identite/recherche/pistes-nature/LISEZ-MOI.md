...
La première série vit dans `identite/recherche/pistes-logo/` et reste disponible : elle cherchait un signe de marque, celle-ci cherche un signe de métier.
...
```bash
node identite/recherche/pistes-nature/build-pistes.mjs     # les 55 fichiers, puis le contrôle de cadrage
node identite/recherche/pistes-nature/build-planche.mjs    # la planche de présentation
```

Le code de fabrication est commun aux deux séries : `identite/recherche/commun/fabrique.mjs` écrit les fichiers, les mesure et monte la planche ; `identite/recherche/commun/artefacts.mjs` compose signe, tuile et lockups ; `identite/recherche/commun/planche.css` habille les deux planches. Une série ne fournit que ses signes, ses repères de construction et son texte, ce qui les empêche de diverger en silence.
